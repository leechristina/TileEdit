...
# Compile and run in the MSYS2 terminal with:
make  
make run

# To Run the Exe Outside the MSYS2 Environment:
Drop the following DLLs inside the TileEditor folder along with TileEdit.exe to run the exe outside the MSYS2 environment:

```
SDL2.dll             libbrotlienc.dll    libjxl_cms.dll     libwebp-7.dll  
SDL2_image.dll       libdav1d-7.dll      liblcms2-2.dll     libwebpdemux-2.dll  
libLerc.dll          libdeflate.dll      liblzma-5.dll      libwinpthread-1.dll  
libSvtAv1Enc-3.dll   libgcc_s_seh-1.dll  libpng16-16.dll    libyuv.dll  
libaom.dll           libhwy.dll          librav1e.dll       libzstd.dll  
libavif-16.dll       libjbig-0.dll       libsharpyuv-0.dll  zlib1.dll  
libbrotlicommon.dll  libjpeg-8.dll       libstdc++-6.dll  
libbrotlidec.dll     libjxl.dll          libtiff-6.dll  
```

And run TileEdit as follows:
...
Artwork is from the following sources under the respective licenses listed below:

## Tilesets from:
**wood_tileset.png**  
Author: Jetrel, Daniel Cook, Zabin  
...
# FONT CREDITS:
**Catamaran-Black.ttf**
https://fonts.google.com/specimen/Catamaran/license
